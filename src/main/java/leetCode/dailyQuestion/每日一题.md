...
要把链表分成k个部分，首先我们可以先得到链表的长度length，那么每部分的长度size应该是length/n。

但是会遇到除不尽的情况，所以我们还需要记录一下余数remainder = length % n.

因为任意两部分的长度差距不能超过1，前面的要比后边的长。所以我们只需要把前remainder部分的size + 1即可。
...
一个数如果是3的幂，那么他的所有因数都是3的幂。

利用这一特性，因为题目给的n最大是2^31 - 1，在这范围内3的最大幂是1162261467。

所以我们只需要判断n是否是1162261467的因数即可。
...
遍历链表，当结点node有子结点node.child时，先保存下node.next，然后将node.child接在node后面。然后继续对node.child进行遍历。

当node.child遍历到链表尾部时，再接上之前保存的node.next即可。
